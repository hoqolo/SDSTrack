...
class SDSTrack(BaseTracker):
    def __init__(self, params):
        super(SDSTrack, self).__init__(params)
        network = build_sdstrack(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            ...
            out_dict = self.network.forward(
                template=self.z_tensor, search=x_tensor, ce_template_mask=self.box_mask_z, infer=True)
