...
import lib.test.parameter.sdstrack as rgbt_prompt_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    if script_name == 'sdstrack':
        ...
        mmtrack = SDSTrack(params)  # "GTOT" # dataset_name
        tracker = SDSTrack_RGBT(tracker=mmtrack)
...
if __name__ == '__main__':
    ...
    if dataset_name == 'GTOT':
        ...
        seq_home = '/public/datasets_neo/RGB-T234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/public/datasets_neo/LasHeR/TestingSet/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
