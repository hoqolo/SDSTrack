...
import lib.test.parameter.sdstrack as rgbe_prompt_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt'):
    ...
    if script_name == 'sdstrack':
        params = rgbe_prompt_params.parameters(yaml_name, epoch)
        sdstrack = SDSTrack(params)  # "VisEvent"
        tracker = SDSTrack_RGBE(tracker=sdstrack)
    ...
    return [seq_name, frame_idx / toc]


class SDSTrack_RGBE(object):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='', help='Name of tracking method(sdstrack, prompt, ftuning).')
    ...
    if args.mode == 'parallel':
        ...
        for seqlist in sequence_list:
            seq_fps = run_sequence(*seqlist)
            seq_fps_list.append(seq_fps)
        with open('fps_rgbe.txt','w') as f:
            fps_sum = 0
            for seq_fps in seq_fps_list:
                f.write(seq_fps[0] + ": " + str(seq_fps[1]) + "\n")
                fps_sum += seq_fps[1]
            f.write("ave:" + str(fps_sum / len(seq_fps_list)))
    ...
    hours = seconds // 3600
    minutes = (seconds % 3600) // 60
    print(f"Totally cost {hours} hours {minutes} minutes")
